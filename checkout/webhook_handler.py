...
class StripeWH_Handler:
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        pid = intent.id
        logger.info(f"Received payment_intent.succeeded event: {pid}")
        logger.info(f"Intent data: {intent}")

        bag = intent.metadata.get('bag')
        if bag is None:
            logger.error(f"Bag data not found in metadata for intent: {pid}")
            return HttpResponse(content="Bag data missing", status=400)

        save_info = intent.metadata.get('save_info') == 'true'

        # Get billing and shipping details
        billing_details = intent.get('charges', {}).get('data', [{}])[0].get('billing_details', intent.get('billing_details', {}))
        shipping_details = intent.get('shipping', {})
        grand_total = round(intent.get('amount', 0) / 100, 2)
        ...
        if order_exists:
            ...
            try:
                ...
                if order:
                    order.delete()
                logger.error(f'Webhook received: {event["type"]} | ERROR: {str(e)}')
                return HttpResponse(
    ...
    def handle_payment_intent_payment_failed(self, event):
        ...
        """
        logger.info(f"Payment failed for intent: {event.data.object.id}")
        return HttpResponse(
